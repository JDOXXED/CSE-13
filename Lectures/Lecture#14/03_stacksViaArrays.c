...
StackOfInt* makeStack(int mxs) {
    StackOfInt *s = (StackOfInt*)malloc(sizeof(StackOfInt));    //allocate memory for stack, s is pointer
    s->mxs = mxs; //Assigns max for stack
    s->sz = 0; //Asigns sign of stack to 0
...
void pushInt(StackOfInt* s, int v) {
    if(s->sz < s->mxs)
        ...
        //If size is equal to capacity, we must allocate more memory
        s->mxs = s->mxs*2; //double the capacity of the stack
        s->t = realloc(s->t, s->mxs * sizeof(int)); //reallocate memory for the stack, double the capacity
        s->t[s->sz++] = v;  //push the element onto the stack
    }
}
