...
void pushBackList(IntList* list, int val) { //inserts a number to the end of the list
...
int popBackList(IntList* list) {    //pop the value at the end of the list
  assert(list->head && list->len > 0); //ensure there is an element to pop, if not then returns an error and ends the program
...
int main(){
  IntList* list = malloc(sizeof(IntList));    //exists on the heap
  initList(list);
  printList(list);
  ...
  freeList(list); //free elements of the list
